...
main = flip evalRandT (mkStdGen 739570) $ do
    ...
    runEffect $
            (reinforcementBatchP runSinSim 10
  ...
  where
    ...
    -- we are using a stochasic policy network, so it will not directly compute sin
    -- instead, it gives us probablities for each discrete slice of the possible outputs for sin
    -- we sample from that output, then plot accordingly
    runSinSim :: Model f g (b, c) b c
                 -> IO [(b, c, Double)]
    runSinSim =
